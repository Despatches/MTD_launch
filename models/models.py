...
basic_deriv_data_pair = [
    ("reference_name", "Name"),
    ("date_first_specified", "Date First Specified"),
    ("ID",),
    ("parent_object_type",),
    ("provisioned_ID",),
    ("parent_ID",),
...
cns += [
    ("registry_name", "Name At registered at Registry"),
    ("percentage", "Share Percentage"),
]

mp_title_derivatives = {
    "c_n_s": {"table": "`companies_and_shares`.`company_share`", "columns": cns},
    "freehold_shares": {
    ...
    "commonholds": {
        ...
        "columns": basic_deriv_data_pair
        + [
            ("percentage_ownership", "Share Percentage"),
            ("percentage_vote", "Vote Percentage"),
        ],
    },
    "freeholds": {
...
def new_form_object(template_set, form_id):
    ...
    new_ob = form_results_collection(
        form_results, template_set["query_select"], form_id, template["form_identifier"]
    )
    return new_ob
...
class market_particulars:
    ...
    def TA6_collection(self):
        self.TA6 = form_data.evaluate_form_mp_relation(
            "TA6_Part_1", "market_particulars", self.id
        )
    ...
    def TA6_subs_collection(self):
        if self.TA6[0] != None:
            ...
            self.TA6 = form_data.form_results_collection(
                "market_particular", self.id, "TA6_Part_1", "ancilliary_form"
            )
    ...
    def fetch_market_particular_components(self, td=mp_title_derivatives):
        ...
        for key in td:
            ...
            query = "SELECT {} FROM {} WHERE parent_ID = %s AND parent_object_type = 'market_particular'".format(
                query_columns, td[key]["table"]
            )
            params = (self.id,)
            cursor.execute(query, params)
            results = cursor.fetchall()
            if results != 0:
                ...
                while col_count < max_cols:
                    if len(columns[col_count]) == 2:
                        self.title_derivatives[key]["headings"].append(
                            columns[col_count]
                        )
                    col_count += 1
                for r in results:
