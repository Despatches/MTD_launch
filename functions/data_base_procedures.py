...
def get_selection_keys(
    sesh_var, answers, a_type, particular_id, selec_set_ID, form_answer
):
    for ans in sesh_var:
...
def delete_last_from_object_queue(
    selec_set_ID, object_selection_set, object_unique_ID, object_ID
):
    delete_from_queue = db.cursor()
    delete_from_queue
    ...
    delete_from_queue.execute(
        f"select max(`prop_order`) into @PO from selection_routines.{object_selection_set} where mp_selection_set_ID = {selec_set_ID}"
    )
    delete_from_queue.execute(query)
    db.commit()
    delete_from_queue.close()
...
def add_to_selections_with_parents_objects(
    answer,
    selec_set_ID,
    parent_selec_ID,
    object_selection_set,
    object_ID_type,
    holding_ID,
):
    answer_input = db.cursor()
    ...
    if max_lft[0] == None or 0:
        add_to_selections_without_parents_objects(
            answer, selec_set_ID, object_selection_set, holding_ID
        )
...
def add_to_selections_without_parents_objects(
    answer, selec_set_ID, object_selection_set, holding_ID
):
    answer_input = db.cursor()
...
def sort_answers_and_add_objects(
    answer,
    selec_set_ID,
    parent_selec_ID,
    object_selection_set,
    object_selection_set_queue,
    object_ID_type,
    holding_ID,
):
    if answer[0] == "no_extra":
        answer = answer[1]
        debug = add_to_selections_with_parents_objects(
            answer,
            selec_set_ID,
            parent_selec_ID,
            object_selection_set,
