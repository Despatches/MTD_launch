...
from flask import Blueprint, render_template, redirect, url_for, request, flash, session,jsonify, json
from launch.blueprints.form_templates.part_wall_entry.Party_wall_entry import party_wall_entry
...
def pw_register_submission():
	...
	for key in inputs:
		equals = pairings[key]
		final_inputs = template_data_transitions.form_equals_evaluation(equals, inputs[key], None)
		print(final_inputs)
...
# opportunity to add irregular data to register e.g user provisioning entry
	...
	print(full_input_set['pairs'])
	generic_input(full_input_set['pairs'], "`party_wall_register`.party_wall_entry_data")
	return 'success'
...
def search_pw_register():
	...
	if request.form.get("filtered") == '1':
		...
		if filters['address_type'] == 'both':
			base_query += "WHERE (psn_postcode = '{postcode}' OR prn_postcode = '{postcode}')".format(postcode=postcode)
		...
		if filters['notice_type'] != 'all':
	 		if len(filters['notice_type']) == 1:
	 			notice_query = "AND party_notice_type ='{}'".format(filters['notice_type'][0])
	 			base_query += notice_query
	 		else:
	 			notice_query = "AND party_notice_type IN ('{}'".format(filters['notice_type'][0])
		...
		base_query += "WHERE (psn_postcode = '{postcode}' OR prn_postcode = '{postcode}')".format(postcode=postcode)
		if address['line_1'] != '':
			base_query += "AND (psn_address_line_1 = '{}' OR prn_address_line_1 = '{}')".format(address['line_1'],address['line_1'])
		if address['line_2'] != '':
			base_query += "AND (psn_address_line_2 = '{}' OR prn_address_line_2 = '{}')".format(address['line_2'],address['line_2'])
		if address['town_or_city'] != '':
			base_query += "AND (psn_postcode_address_town_or_city = '{}' OR prn_postcode_address_town_or_city = '{}')".format(address['town_or_city'],address['town_or_city'])

	cursor = db.cursor()
	collect_set = ['form_id','psn_postcode', 'psn_address_line_1' , 'psn_address_line_2', 'psn_postcode_address_town_or_city', 'psn_UPRN', 'prn_postcode', 'prn_address_line_1', 'prn_address_line_2', 'prn_postcode_address_town_or_city', 'prn_UPRN', 'party_notice_type', 'date_of_service']
...
def search_pw_records(notice):
	...
	notice = template_data_transitions.form_results_collection(template_data_transitions.collect_form_data(party_wall_entry['query_columns'],notice, "`party_wall_register`.party_wall_entry_data"),party_wall_entry['query_select'], notice, form_name)

	return render_template('party_wall_register/view_notice.html', notice=notice, json_notice=json.dumps(notice.__dict__))
