...
@TA6_forms.route("/create_form_db_table", methods=['POST'])
def create_form_db_table():
	def characteristics_of_risk(risk_list):
		new_list = copy.deepcopy(risk_list)
		for key in new_list:
			for value in new_list[key]:
				new_list[key][value] = {}
				for risk in risk_list[key][value]:
					new_list[key][value][risk] = {'indemnity_limit':'static', 'effect_description':''}
		return new_list


	def ident_fetch(template):
		idents = []
		ident_sets
		def ident_fetch(loopy):
			for q in loopy:
				if ('question_set' not in q) and q['input_type'] != 'multi_row':
					ident_data = {}
					ident_marker = {q['identifier']:ident_data}
					idents.append(ident_marker)

				elif 'question_set' in q:
					ident_fetch(q['sub_questions'])

				elif q['input_type'] == 'multi_row':
					ident_set_data = []
					ident_set =  {q['identifier']:ident_set_data}

	call = request.form.get('type')
	...
	tp = {}
	exec(tempy, tp)
	template = template_sort(tp['template'])['template']
	db_area = 'form_data'
	if call == 'db':
		db_area = 'form_data'

		sql = create_new_sql_table(template, db_area)
		return sql
	
	elif call == 'idents':
		ident_fetch(template)

	elif call == 'indem_risk':
		for sets in tp:
	...
	"""elif call == 'gpt_training':
		form = request.form.get('template')
		prompts_set=[]
		if form in templates:
			form_set = templates[form]
			results = template_sort(form_set['template'], True)
			for section in results['sections']:
				for q in results['sections'][section]:
					new_prompt = {'prompt':'completion':''}
					prompt = 'question: {identifier}'.format(**q)
					if 'other_question_text' in q:
						prompt += 'other_question_text:{other_question_text}'.format(**q)
						for flow in 
							reliance_prompt = ''
							for display in 'display_reliance':
								for ans in display['value']:
									if template_set['meanings'][q['identifier']][output_py_return(ans, )]
									#reliance_prompt +="""
...
def bulk_form_collect(form, template_set, *args):
	template = template_set['template']
	form_results = form_results_collection(collect_form_data(template_set['query_columns'],form), form, template['form_identifier'])
...
def collect_synopsis_data(form_name,p_type, particular_id):
	if form_name == 'TA6_Par_1':
		...
		if results[1] == 'old_form':
			...
			return redirect(url_for(TA6_Part_1(p_type, particular_id, form = [form,'new'])))
	else:
...
def create_work_task():
	...
	work_task_set = (work_task['form_name'], work_task['form'], work_task['identifier'], work_task['work_task_header'], work_task['work_task_date'], work_task['work_task_info'], work_task['client_issue'])
...
def form_data_prefil(form, form_set, template):
	if form['form_age'] == 'old_form':
		...
		return {'result_send':result_send, 'sub_tables_send':sub_tables_send, 'section_marker':form_results.section_marker}
	else:
		return {'result_send':json.dumps('none'), 'sub_tables_send':json.dumps('none'),'section_marker':0}
...
def template_micro_form(form_name, root_linkage, root_linkage_id, **kwargs):
	form_set = templates[form_name]
	main_form_data = template_send_prep(form_name)
	form = form_results_collection(root_linkage, root_linkage_id,form_name, 'ancilliary_form' )
	main_form_data['form_id'] = form.form
	main_form_data['template'] = form_set['template']
	if form.data == None:
		main_form_data['result_send'] = json.dumps('none')
	else:main_form_data['result_send'] = json.dumps(form.data)
...
def micro_submission():
	...
	form = form_results_collection(linkage['root_linkage'], linkage['root_linkage_id'],linkage['form_name'], 'ancilliary_form' )
...
def micro_synop_return(data):
	template_set = templates[data['form_name']]
	template = template_set['template']
	form = form_results_collection(data['root_linkage'], data['root_linkage_id'],data['form_name'], 'ancilliary_form' )
