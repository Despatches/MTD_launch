	...
	def set_loop(sets):
		...
		for question in sets:
			...
			if question['input_type'] == 'multi_row':
				equiv_list.append({'TA6':'', 'TA6_part_1':{'identifier':question['identifier']}})
				set_loop(question['data_rows'])
			elif 'question_title' in question:
				equiv_list.append({'TA6':'', 'TA6_part_1':{'identifier':question['identifier'], 'question_title':question['question_title']}})
			else: equiv_list.append({'TA6':'', 'TA6_part_1':{'identifier':question['identifier']}})
...
def print_text_modifier(template):
	...
	def set_loop(sets):
		...
		for question in sets:
			n += 1
			if question['input_type'] == 'multi_row':
				equiv_list[question['identifier']] = {'question_title':None, 'other_question_text':None}
				set_loop(question['data_rows'])
			else: equiv_list[question['identifier']] = {'question_title':None, 'other_question_text':None}
...
class form_results_collection:
		...
		if collection_type == 'query_pairings':
			...
			query = """SELECT 
				...
				AND form_id=(
		   			 ...
		   			 WHERE form_identifier = '{form_name}' AND root_linkage = '{root_linkage}' AND root_linkage_id = {root_linkage_id}
		   		);""".format(form_name=form_name, root_linkage=root_linkage, root_linkage_id=root_linkage_id)
	...
	def micro_upload_section(self, section_data, section,sec_status):
		...
		params = (json.dumps(self.results), self.revision_count+1,self.section_marker, self.form_name, self.root_linkage, self.root_linkage_id )
	...
	def add_forms(self, form_identifier, root_linkage, root_linkage_id):
		...
		if len(results) > 0:
			...
			for res in results:
				self.sub_forms[form_identifier].append(form_results_collection(res[1],res[0],form_identifier, 'ancilliary_forms', section_marker=res[2], user_initiator= res[3], creation_moment=res[3]))
				...
				for key in templates[self.form_name]['comp_risk_fraud'][q]:
					for groupy in crf:
						if f'{groupy}_potential' not in self.data[q]:
							if templates[self.form_name]['comp_risk_fraud'][q][key][groupy] == 1:
...
# assign any colleted detail text for questions with the inoput type of detail_text
	def fetch_detail_data(self):
		cursor = db.cursor()
		query = "SELECT text_content, ref, text_reference, viewed FROM `form_data`.`{}_detail_text` WHERE parent_form= %s;".format(self.form_name)
		params = (self.form,)
	...
	def edited_text_meaning(self):
		if self.collection_type != 'micro':
			...
			query = """SELECT `reference_edit`, `editation_data` FROM {}
						WHERE `parent_form` =  {}""".format(table, self.form)
			cursor =  db.cursor()
			cursor.execute(query)
...
# currently just for the purpose of answer meanings allowing answer meanings to be tagged to be assigned based on question answer
	def append_data(self, data_set, data_name):
		for key in data_set:
			if key in self.data:
				identifier = key
				collected_value = output_py_return(collected_value_interpretation(self.data[identifier]['value']),self.data[identifier]['input_type'], reverse=True)
				if collected_value in data_set[identifier]:
					self.data[identifier][data_name] = data_set[identifier][collected_value]
				elif 'value' in data_set[identifier] and collected_value != 'none':
					if 'equation' in data_set[identifier]['value']:
						new_meaning = None
						for key in data_set[identifier]['value']['equation']:
							result = key(self.data[identifier]['value'], data_set[identifier]['value']['equation'][key]['vari'])
							if result == True:
								new_meaning = data_set[identifier]['value']['equation'][key]['meaning']
								
								break
						self.data[identifier][data_name] = {}
						if new_meaning == None:
							self.data[identifier][data_name]['meaning'] = data_set[identifier]['value']['default']['meaning']
						else:
							self.data[identifier][data_name]['meaning'] = new_meaning
		
					else:
						meaning_string = data_set[identifier]['value']['meaning']
						if meaning_string != "":
							self.data[identifier][data_name] = {}
							self.data[identifier][data_name]['meaning'] = meaning_string.format(self.data[identifier]['value'])
...
# apply extra data to set based off identifiers for example risk, fraud or competancy markers
	def append_data_group(self, data_set):
		for key in data_set:
			if key in self.data:
				identifier = key
				collected_value = collected_value_interpretation(self.data[identifier]['value'])
				if collected_value in data_set[identifier]:
					for key in data_set[identifier][collected_value]:
						self.data[identifier][key] = data_set[identifier][collected_value][key]
				elif 'value' in data_set[identifier] and collected_value != 'none':
					for key in data_set[identifier][collected_value]:
						self.data[identifier][key] = data_set[identifier][collected_value][key]
		...
		'''def evaluate_characteristics(self, **kwargs):
			for key in self.data:
				self.[key] = result(key, self.data[key], kwargs)

	#def master_flow_control(self):
		#for key in self.data:
...
#collect and assign data that belongs to a tmeplate specific object
	def find_documents(self, gather=False):
		...
		query = """SELECT ref, text_reference, document_name, document_description,viewed, upload{}
					...
					WHERE parent_form = {form_id}""".format(gathering, form_name = self.form_name, form_id = self.form)
		...
		for doc in documents:
			for key in self.data:
				if key == doc[1]:
					new_doc = {'ref':doc[0],'document_name' :doc[2], 'document_description':doc[3],'viewed':doc[4],'upload_date':doc[5]}
					if gather == True:
			...
			if cur_obj['object'] in objects:
				...
				if allowed == True:
					...
					if cur_obj['position'] == 'first':
						self.element_relevances['questions'][cur_obj['section']] = [{'object':key}] + self.element_relevances['questions'][cur_obj['section']]
...
def input_type_jiggling(e, data, form, docu_det, ident):
	...
	if data['input_type'] == 'bool' or data['input_type'] == 'bool_extra':
		...
		if val != False:
			db_ready_value = f"'{val}'"
		else: db_ready_value = repr('empty')

	elif data['input_type'] == 'text':
		if val != False:
			db_ready_value = f"'{val}'"
		else: db_ready_value = 'NULL'
...
# if input reference name is referenced in java and python
					if sub_row_matches['form'] in r and r[sub_row_matches['form']]['value'] != False:
						...
						if r[sub_row_matches['form']]['input_type'] in alter_functions:
							data_piece = alter_functions[r[sub_row_matches['form']]['input_type']](returns[0])
						else:
			...
			#print(multi_pairs)
			query = multi_line_input(multi_pairs, e['table'], form = {'row':'parent_form', 'id':form})
			...
			if len(sub_table_docu_det['details']) > 0:
				...
				cursor.execute(f'UPDATE {table} set sub_attributes = TRUE WHERE parent_form = {form} AND sub_attributes = FALSE')
...
def multi_line_input(pairs, table, **kwargs):
	...
	for p in pairs:
		...
		if u_num == 1:
			...
			row_input_counter = 0
			while row_input_counter < row_input_num:
				row_vals[row_input_counter] = f"{row_vals[row_input_counter]}, {p['value'][row_input_counter]} "
				row_input_counter +=1
			...
			for val in control['value']:
				if val == comp:
					...
					if control['identifier'] in control_set:
						...
						item_relevant = flow_control_tiers(control_set[control['identifier']], data)
...
def form_equals_evaluation(equals, form_data, form):
	...
	for e in equals:
		if e['form'] in form_data:
			...
			ident = e['form']
			returns = input_type_jiggling(e, data, form, {'docu':docu, 'details':details}, ident)
