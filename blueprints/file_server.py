from flask import Blueprint,Flask, make_response, request, session, render_template, send_file, Response
...
root = os.path.normpath("/Volumes/MACINTOSH/MTD/MTD_code/Main_Client_Interface/Refined_project_development/launch/templates")
key = ""
...
@file_server.app_template_filter('size_fmt')
def size_fmt(size):
    return humanize.naturalsize(size)

@file_server.app_template_filter('time_fmt')
def time_desc(timestamp):
    mdate = datetime.fromtimestamp(timestamp)
    str = mdate.strftime('%Y-%m-%d %H:%M:%S')
    return str

@file_server.app_template_filter('data_fmt')
...
@file_server.app_template_filter('icon_fmt')
...
@file_server.app_template_filter('humanize')
...
class PathView(MethodView):
    def get(self, p=''):
        ...
        if os.path.isdir(path):
            ...
            page = render_template('file_server_index.html', path=p, contents=contents, total=total, hide_dotfile=hide_dotfile)
    ...
    def put(self, p=''):
        if request.cookies.get('auth_cookie') == key:
            ...
            res.headers.add('Content-type', 'file_serverlication/json')
        else:
...
file_server.add_url_rule('/file_server', view_func=path_view)
file_server.add_url_rule('/file_server/<path:p>', view_func=path_view)
...
"""if __name__ == '__main__':
    ...
    port = os.getenv('FS_PORT', '5000')
    root = os.path.normpath(os.getenv('FS_PATH', '/tmp'))
    file_server.run(bind, port, threaded=True, debug=False)"""
